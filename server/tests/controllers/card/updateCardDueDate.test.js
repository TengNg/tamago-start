...
describe('PATCH /cards/:id/new-due-date', () => {
    ...
    beforeEach(async () => {
        ...
        mockRequestBody = {
            // make sure it's in UTC, since mongo will store it in UTC
            dueDate: new Date("2026-04-01T16:17:13.770Z")
        }
    });
    ...
    it('should return 404 if id not found', async () => {
        ...
        const res = await request(app)
            .patch(`/api/cards/${unknownId}/new-due-date`)
    ...
    it('should return 403 if user does not have access to board', async () => {
        ...
        const res = await request(app)
            .patch(`/api/cards/${anotherCard._id}/new-description`)
            .set('Cookie', `${cookieName}=${accessToken}`)
            .send(mockRequestBody)
    ...
    it('should return 200 if id is found', async () => {
        const res = await request(app)
            .patch(`/api/cards/${testCard._id}/new-due-date`)
            .set('Cookie', `${cookieName}=${accessToken}`)
            .send(mockRequestBody)
