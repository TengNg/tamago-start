...
describe('PATCH /cards/:id/new-description', () => {
    ...
    it('should return 404 if id not found', async () => {
        ...
        const res = await request(app)
            .patch(`/api/cards/${unknownId}/new-description`)
    ...
    it('should return 200 if id is found', async () => {
        const res = await request(app)
            .patch(`/api/cards/${testCard._id}/new-description`)
            .set('Cookie', `${cookieName}=${accessToken}`)
            .send(mockRequestBody)
        expect(res.statusCode).toBe(200);
        expect(res.body.newCard.description).toBe(mockRequestBody.description);
    });
});
