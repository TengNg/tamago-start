...
describe('PATCH /cards/:id/new-owner', () => {
    ...
    it('should return 404 if id not found', async () => {
        ...
        const res = await request(app)
            .patch(`/api/cards/${unknownId}/new-owner`)
    ...
    it('should return 403 if user does not have access to board', async () => {
        ...
        const res = await request(app)
            .patch(`/api/cards/${anotherCard._id}/new-owner`)
            .set('Cookie', `${cookieName}=${accessToken}`)
            .send(mockRequestBody)
    ...
    it('should return 200 if id is found', async () => {
        const res = await request(app)
            .patch(`/api/cards/${testCard._id}/new-owner`)
            .set('Cookie', `${cookieName}=${accessToken}`)
            .send(mockRequestBody)
        expect(res.statusCode).toBe(200);
        expect(res.body.newCard.owner).toBe(mockRequestBody.ownerName);
    });
});
