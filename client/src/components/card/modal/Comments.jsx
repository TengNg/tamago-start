...
import {
    fetchComment as fetchCommentApi,
...
const Comments = ({ card }) => {
    ...
    useEffect(() => {
        ...
        const fetchFocusedComment = async (id) => {
            try {
                const data = await fetchCommentApi(card._id, id);
                setFocusedComment({
                    ...data.comment,
                    collapsed: false,
                });
    ...
    const fetchComments = async ({ page = 1 }) => {
        return await fetchCommentsApi(card._id, { page });
    };
    ...
    const addComment = async (content) => {
        const data = await addCommentApi(card._id, content);
        return data?.comment;
    };
    ...
    const deleteComment = async (commentId) => {
        await deleteCommentApi(card._id, commentId);
    };
